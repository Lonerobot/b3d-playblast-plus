...
- It calls `subprocess.call(cmd, shell=True)` in `lib/encode.py`, which is a potential
  shell-injection risk when paths contain spaces or special characters (see §4.1).
...
### 4.1 Shell-Injection Risk in `lib/encode.py` (`mp4_from_image_sequence`)
...
If `ffmpeg_path`, `image_seq_path`, or `output_path` contains shell metacharacters
(e.g. a file named `shot$(rm -rf ~).png`), the shell will interpret them.
`apng_from_image_sequence` avoids this by passing a list to `subprocess.run`
(no `shell=True`) — the same approach should be applied to `mp4_from_image_sequence`.

**Recommendation:** Convert `mp4_from_image_sequence` to use a list-based `subprocess.run`
call, removing `shell=True`.
...
publish/
├── __init__.py
├── create_playblast.py          # Creator — registers a "playblast" instance
├── collect_playblast.py         # Collector — enriches instance with AYON context
├── validate_frame_range.py      # Validator — start ≤ end, non-zero
├── validate_camera.py           # Validator — active camera exists
├── validate_output_dir.py       # Validator — output path is writable
├── extract_playblast.py         # Extractor — captures PNG sequence + encodes
└── integrate_review.py          # Integrator — registers review media with AYON
```
...
class CreatePlayblast(pyblish.api.ContextPlugin):
    """Register a viewport playblast as a publishable instance."""
    ...
    def process(self, context):
        import bpy
        instance = context.create_instance("playblast")
        instance.data["family"]      = "review"
        instance.data["productType"] = "review"
        instance.data["productName"] = "playblast"
...
class ExtractPlayblast(pyblish.api.InstancePlugin):
    ...
    def process(self, instance):
        ...
        # ... run capture + encode into staging_dir ...
...
|---|---|---|---|
| 🔴 High | Fix shell-injection risk (`shell=True`) | `lib/encode.py` | §4.1 |
...
| 🔵 Future | Implement AYON publish pipeline plugins | `publish/` | §5 |
...
a useful starting point, but it does not yet participate in the AYON publish pipeline.
Addressing the high-priority code issues (shell injection, frame range validation) is
recommended regardless of whether full AYON publish integration is pursued. If AYON
publish integration is a goal, the architectural sketch in §5 provides a path that aligns
with the principles documented at https://docs.ayon.dev/docs/dev_publishing/.
